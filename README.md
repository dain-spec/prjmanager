...
업무 요청은 같은 파일 정보를 두 번 적지 않는다. 유형 · 파일 경로 · 제플린을
비워 두면 **서비스명 + OS** 로 파일 현황 행을 찾아 그 값을 보여 준다.

- **OS 까지 봐야 행 하나가 정해진다.** ONE AI(W/M) · 메신저(W/C) · 화상회의(W/M)
  처럼 OS 만 다른 서비스가 있어 서비스명만으로는 유일하지 않다.
- 파일 현황에 **메뉴명이 채워진 행이 여럿이면 '메뉴명이 빈 행(= 서비스 전체)'**
  을 먼저 쓰고, 없으면 첫 행을 쓴다.
- **직접 적은 값이 이긴다.** 요청은 서비스 대표 파일이 아니라 특정 화면을
  가리키는 경우가 많고, 끝난 요청의 링크가 나중에 파일 현황이 바뀌었다고
  따라 바뀌면 곤란하다. 편집 중에는 가져올 값이 입력칸의 힌트로 보이고,
  유형은 '자동' 을 고르면 비워 둔 것으로 친다.
- 가져온 값은 흐린 글자(배지는 채움 없이 테두리만)로 구분하고, 마우스를 올리면
  '파일 현황에서 가져온 값입니다' 가 뜬다.
- 이름이 정확히 같아야 연결되므로 업무 요청의 서비스명은 파일 현황에 등록된
  이름을 자동완성으로 제안한다.
