...
| **업무 요청** | 부서에서 들어온 업무 요청의 처리 현황 | 요청일 · 요청자 · 요청내용 · 쪽지 · 서비스명 · OS · 유형 · 파일 경로 · zep · 담당자 · 완료일 |
...
| 서비스 묶음 | 이름이 달라도 한 덩어리로 볼 서비스는 `src/app.js` 의 `SERVICE_FAMILIES` 에 적는다. 지금은 AI 제품군(ProActive AI · ONE AI · ONE AI CUBE · ONE AI Flow · Agent Market) 하나가 등록되어 있다 |
...
| 순서 변경 | No 칸을 드래그 |
| 컬럼 폭 | 모든 컬럼을 헤더 경계 드래그로 조절. 탭마다 따로 기억하고, 더블클릭하면 기본값 |
| 남는 폭 | 표 오른쪽 여백 칸이 흡수하되, 폭을 직접 정하기 전까지는 비고 · 요청내용이 남는 폭을 가져가 화면을 채운다 |
